...
class baseClass {
  ...
  async syncDevices({ devices, version = "1" }) {
    ...
    console.log("v1 syncDevices ===>", JSON.stringify(request));
    return await this.httpRequest({
  ...
  async initSSE() {
    ...
    console.log(
      "🚀 ~ file: baseClass.ts:320 ~ baseClass ~ initSSE ~ url -- v1",
      url
    );
    try {
  ...
  mountSseFunc(handler) {
    ...
    handler.onAddDevice &&
      this.event.addEventListener("device#v1#addDevice", handler.onAddDevice);
    handler.onUpdateDeviceState &&
      this.event.addEventListener(
        "device#v1#updateDeviceState",
        handler.onUpdateDeviceState
      );
    handler.onUpdateDeviceInfo &&
      this.event.addEventListener(
        "device#v1#updateDeviceInfo",
        handler.onUpdateDeviceInfo
      );
    handler.onUpdateDeviceOnline &&
      this.event.addEventListener(
        "device#v1#updateDeviceOnline",
        handler.onUpdateDeviceOnline
      );
    handler.onDeleteDevice &&
      this.event.addEventListener(
        "device#v1#deleteDevice",
        handler.onDeleteDevice
      );
    handler.onSecurityChange &&
      this.event.addEventListener(
        "security#v1#updateArmState",
        handler.onSecurityChange
      );
    handler.onAlarmChange &&
      this.event.addEventListener(
        "bridge#v1#updateAlarmState",
        handler.onAlarmChange
      );
  }
  unmountSseFunc() {
    ...
    this.event?.removeEventListener("security#v1#updateArmState", () => {});
    this.event?.removeEventListener("bridge#v1#updateAlarmState", () => {});
    this.event?.close();
  }
}
