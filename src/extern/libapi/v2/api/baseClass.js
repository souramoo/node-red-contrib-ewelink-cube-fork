...
class baseClass {
  ...
  async getBridgeATHandler() {
    const params = { app_name: "node-red-contrib-ewelink-cube-fork" };
    // console.log('----->', new Date().getTime() - this.time);
    const resp = await this.httpRequest({
      path: EPath_1.default.BRIDGE_TOKEN,
      method: EMethod_1.default.GET,
      isNeedAT: false,
      params,
    });
    if (resp.error === 0) {
  ...
  async updateBridgeConfig(volume) {
    return await this.httpRequest({
      path: EPath_1.default.BRIDGE_CONFIG,
  ...
  async rebootBridge() {
    return await this.httpRequest({
      path: EPath_1.default.HARDWARE_REBOOT,
      method: EMethod_1.default.POST,
    });
  }
  /**
   * 扬声器控制
  ...
  async controlSpeaker(params) {
    return await this.httpRequest({
      path: EPath_1.default.HARDWARE_SPEAKER,
      method: EMethod_1.default.POST,
      params,
    });
  }
  ...
  async syncDevices({ devices, version = "2" }) {
    if (version !== "2")
      return { error: 1000, msg: "version must be 2", data: {} };
    const request = {
      event: {
        header: {
    ...
    return await this.httpRequest({
      path: EPath_1.default.THIRD_PARTY,
      method: EMethod_1.default.POST,
      params: request,
    });
  }
  ...
  async getDebugLog({ serial_number, ...params }) {
    return await this.httpRequest({
      path: `${EPath_1.default.DEBUG_LOG}/${serial_number}`,
      method: EMethod_1.default.GET,
      params,
    });
  }
  ...
  async httpRequest(httpConfig) {
    ...
    try {
      ...
      this.debug &&
        console.log("http response body---->", JSON.stringify(resp.data));
      return resp.data;
    } catch (error) {
  ...
  async initSSE() {
    ...
    console.log(
      "🚀 ~ file: baseClass.ts:320 ~ baseClass ~ initSSE ~ url -- v2",
      url
    );
    try {
      ...
      console.log(
        "🚀 ~ file: baseClass.ts:323 ~ baseClass ~ initSSE ~ error",
        error
      );
    }
  }
  mountSseFunc(handler) {
    ...
    handler.onAddDevice &&
      this.event.addEventListener("device#v2#addDevice", handler.onAddDevice);
    handler.onUpdateDeviceState &&
      this.event.addEventListener(
        "device#v2#updateDeviceState",
        handler.onUpdateDeviceState
      );
    handler.onUpdateDeviceInfo &&
      this.event.addEventListener(
        "device#v2#updateDeviceInfo",
        handler.onUpdateDeviceInfo
      );
    handler.onUpdateDeviceOnline &&
      this.event.addEventListener(
        "device#v2#updateDeviceOnline",
        handler.onUpdateDeviceOnline
      );
    handler.onDeleteDevice &&
      this.event.addEventListener(
        "device#v2#deleteDevice",
        handler.onDeleteDevice
      );
    handler.onSecurityChange &&
      this.event.addEventListener(
        "security#v2#updateArmState",
        handler.onSecurityChange
      );
    handler.onAlarmChange &&
      this.event.addEventListener(
        "bridge#v2#updateAlarmState",
        handler.onAlarmChange
      );
  }
  unmountSseFunc() {
