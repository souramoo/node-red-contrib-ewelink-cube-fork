...
class baseClass {
    ...
    async getBridgeATHandler() {
        const params = { app_name: 'node-red-contrib-ewelink-cube' };
        // console.log('----->', new Date().getTime() - this.time);
        const resp = await this.httpRequest({ path: EPath_1.default.BRIDGE_TOKEN, method: EMethod_1.default.GET, isNeedAT: false, params });
        if (resp.error === 0) {
            this.interval && clearInterval(this.interval);
            //	set at
     ...
     * 同步新设备列表
     */
    async syncDevices({ devices, version = '2' }) {
        if (version !== '2') return { error: 1000, msg: 'version must be 2', data: {} };
        const request = {
            event: {
                header: {
        ...
        console.log('v2 syncDevices ===>', JSON.stringify(request));
        return await this.httpRequest({ path: EPath_1.default.THIRD_PARTY, method: EMethod_1.default.POST, params: request });
    }
    /**
     * 设备状态更新上报
     */
    async uploadDeviceState({ serial_number, third_serial_number, params, version = '2' }) {
        if (version !== '2') return { error: 1000, msg: 'version must be 2', data: {} };
        const request = {
            event: {
                header: {
     ...
     * 设备上下线状态上报
     */
    async updateDeviceOnline({ serial_number, third_serial_number, params, version = '2' }) {
        if (version !== '2') return { error: 1000, msg: 'version must be 2', data: {} };
        const request = {
            event: {
                header: {
    ...
    /**
     * 启用指定安防模式
     * @param sid security_id
     */
    async enableSecurityById(sid) {
        return await this.httpRequest({
            method: 'put',
            path: `/security/${sid}/enable`,
        });
    }
    /**
     * 禁用指定安防模式
     * @param sid security_id
     */
    async disableSecurityById(sid) {
        return await this.httpRequest({
            method: 'put',
            path: `/security/${sid}/disable`,
        });
    }
    /**
     * 一键布防
     */
    async enableAllSecurity() {
        return await this.httpRequest({
            method: 'put',
            path: '/security/enable',
        });
    }
    /**
     * 一键撤防
    ...
    async initSSE() {
        ...
        console.log('🚀 ~ file: baseClass.ts:320 ~ baseClass ~ initSSE ~ url -- v2', url);
        try {
    ...
    mountSseFunc(handler) {
        ...
        handler.onAddDevice && this.event.addEventListener('device#v2#addDevice', handler.onAddDevice);
        handler.onUpdateDeviceState && this.event.addEventListener('device#v2#updateDeviceState', handler.onUpdateDeviceState);
        handler.onUpdateDeviceInfo && this.event.addEventListener('device#v2#updateDeviceInfo', handler.onUpdateDeviceInfo);
        handler.onUpdateDeviceOnline && this.event.addEventListener('device#v2#updateDeviceOnline', handler.onUpdateDeviceOnline);
        handler.onDeleteDevice && this.event.addEventListener('device#v2#deleteDevice', handler.onDeleteDevice);
        handler.onSecurityChange && this.event.addEventListener('security#v2#updateArmState', handler.onSecurityChange);
        handler.onAlarmChange && this.event.addEventListener('bridge#v2#updateAlarmState', handler.onAlarmChange);
    }
    unmountSseFunc() {
        this.event?.removeEventListener('device#v2#addDevice', () => {});
        this.event?.removeEventListener('device#v2#updateDeviceState', () => {});
        this.event?.removeEventListener('device#v2#updateDeviceInfo', () => {});
        this.event?.removeEventListener('device#v2#updateDeviceOnline', () => {});
        this.event?.removeEventListener('device#v2#deleteDevice', () => {});
        this.event?.removeEventListener('security#v2#updateArmState', () => {});
        this.event?.removeEventListener('bridge#v2#updateAlarmState', () => {});
        this.event?.close();
    }
}
